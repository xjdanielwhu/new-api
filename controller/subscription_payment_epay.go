...
func SubscriptionEpayReturn(c *gin.Context) {
	...
	if c.Request.Method == "POST" {
		// POST 请求：从 POST body 解析参数
		if err := c.Request.ParseForm(); err != nil {
			c.Redirect(http.StatusFound, service.PaymentReturnURL("/wallet?pay=fail"))
			return
		}
		params = lo.Reduce(lo.Keys(c.Request.PostForm), func(r map[string]string, t string, i int) map[string]string {
			r[t] = c.Request.PostForm.Get(t)
	...
	if verifyInfo.TradeStatus == epay.StatusTradeSuccess {
		...
		c.Redirect(http.StatusFound, service.PaymentReturnURL("/wallet?pay=success"))
		return
	}
	c.Redirect(http.StatusFound, service.PaymentReturnURL("/wallet?pay=pending"))
}
