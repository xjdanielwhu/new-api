...
// writeEpayNotifyResponse 在业务处理完成后回写易支付 webhook 响应。
// 只有确认订单与额度均已落库才返回 success，否则返回 fail 让支付平台按其重试策略重新投递，
// 避免提前 ACK 导致支付成功但额度未入账且不再重试。
func writeEpayNotifyResponse(c *gin.Context, ok bool) {
	body := "fail"
	if ok {
		body = "success"
	}
